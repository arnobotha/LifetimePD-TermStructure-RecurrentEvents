...
# Precompute the total number of markers
n <- length(dat$Marker)

# Preallocate storage for weights
weights <- matrix(0, nrow = n, ncol = length(uMarker))

# Loop through unique markers
for (j in seq_along(uMarker)) {
  # Calculate differences
  Diff <- dat$Marker - uMarker[j]
  sorted_indices <- order(Diff)  # Sort indices of Diff
  sorted_Diff <- Diff[sorted_indices]  # Sort Diff
  
  # Determine indices and bounds
  index0 <- sum(sorted_Diff < 0) + 1
  index1 <- min(index0 + trunc(n * span + 0.5), n)
  lambda <- sorted_Diff[index1]
  
  # Assign weights for positive bounds
  wt <- (Diff <= lambda) & (Diff >= 0)
  
  # Adjust for symmetric bounds
  index0 <- sum(sorted_Diff <= 0)
  index2 <- max(index0 - trunc(n * span / 2), 1)
  lambda <- abs(sorted_Diff[index1])
  
  # Assign weights for symmetric range
