...
pack.ffdf(paste0(genPath,"cox_TFD"), cox_TFD)
...
ptm <- proc.time() #IGNORE: for computation time calculation
survFit_pred_TFD <- survfit(cox_TFD, centered=F, newdata=datCredit_valid_TFD, id=PerfSpell_Key)
proc.time() - ptm # 86m
...
datHaz <- survQuants.data(datGiven_train=datCredit_train_TFD, datGiven_score=test, 
