...
#   - datMV | Features created on a subset of fields in macro_data_hist
# ---------------------------------------------------------------------------------------
...
# Detected error in previous code
...
datMV <- macro_data_hist[,list(Date=Date_T, Repo_Rate_I, Inflation_I, DTI_I, Employment_I, 
...
datMV[, Date := as.Date(Date, format="%Y-%m-%d")]
...
cor(datMV$Repo_Rate_I, datMV$M_Repo_Rate)
# Correlation - 100%
cor(datMV$Inflation_I, datMV$M_Inflation_Growth)
# Correlation - 99.99999%
cor(datMV$DTI_I, datMV$M_DTI_Growth)
# Correlation - 100%
cor(datMV$Employment_I, datMV$M_Emp_Growth)
# Correlation - 99.99992%
cor(datMV$RealGDP_I, datMV$M_RealGDP_Growth)
# Correlation - 99.99993%
cor(datMV$RealIncome_I, datMV$M_RealIncome_Growth)
# Correlation - 99.99995%
...
datMV[nrow(datMV), names(datMV[,2:ncol]) := datMV[nrow(datMV) - 1,2:ncol]] # Fill up blank row with previous row's values

# Safety Check
all(datMV[nrow,2:ncol] == datMV[nrow-1,2:ncol])# Should be TRUE
...
datMV[, (colNames) := lapply(1:length(colNames), function(i, dat, colNames) { 
  # testing conditions:
  # i <- 1; dat <- datMV;
...
datMV[, M_Inflation_Growth_Vol := M_Inflation_Growth]
datMV[, M_DTI_Growth_Vol := M_DTI_Growth]
datMV[, M_Emp_Growth_Vol := M_Emp_Growth]
datMV[, M_RealGDP_Growth_Vol := M_RealGDP_Growth]
datMV[, M_Repo_Rate_Vol := M_Repo_Rate]
datMV[, M_RealIncome_Growth_Vol := M_RealIncome_Growth]
...
datMV[, (colNames_vol) := lapply(1:length(colNames_vol), function(i, dat, colNames_vol) { 
  # testing conditions:
  # i <- 1; dat <- datMV;
...
suppressWarnings(datMV[, `:=`(M_Inflation_Growth_Vol_1 = NULL, M_DTI_Growth_Vol_1 = NULL, 
...
pack.ffdf(paste0(genPath, "datMV"), datMV)
