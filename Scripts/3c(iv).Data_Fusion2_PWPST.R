# ================================ DATA FUSION - PWPST ====================================
# Subsample the full PWPST credit dataset before fusing it to the wider "input space" data.
...
#   - datCredit_PWPST | prepared credit data from script 3b
...
if (all(is.na(stratifiers))){ # No stratifiers
  ...
  dat_keys <- unique(datCredit_PWPST[, mget(c(clusVar))])
  ...
  # - Get unique loan account IDs from the full dataset
  dat_keys <- unique(datCredit_PWPST[, mget(c(clusVar, stratifiers))])
...
pack.ffdf(paste0(genPath,"creditdata_final_PWPST_keys"), dat_smp_keys)

# - Obtain the associated loan records as to create the subsampled dataset
datCredit_smp <- copy(datCredit_PWPST[get(clusVar) %in% dat_smp_keys[, get(clusVar),]])
...
# - Save intermediary snapshot to disk (zip) for quick disk-based retrieval later
pack.ffdf(paste0(genPath,"creditdata_final_PWPST_smp1c"), datCredit_smp)
...
pack.ffdf(paste0(genPath,"creditdata_final_PWPST_smp1d"), datCredit_smp)
...
if (!exists('dat_smp_keys')) unpack.ffdf(paste0(genPath,"creditdata_final_PWPST_keys"), tempPath)
...
# check.3 <- length(unique(datCredit_smp$LoanID)) == length(unique(datCredit_train_PWPST$LoanID)) + length(unique(datCredit_valid_PWPST$LoanID))
