# ================================ DATA FUSION - AG ====================================
# Subsample the full AG credit dataset before fusing it to the wider "input space" data.
...
#   - datCredit_AG | prepared credit data from script 3b
#   - various parameters set in the setup script 0
#   - datMV | prepared feature engineered macroeconomic data from script 2a
#
# -- Outputs:
#   - datCredit_smp   | subsampled dataset of the enriched datCredit_AG
#   - datCredit_train_AG | Training set for AG-definition of Cox PH model
#   - datCredit_valid_AG | Validation set for AG-definition of Cox PH model
# ---------------------------------------------------------------------------------------
...
cat(smp_size, " is ", sprintf("%.4f", smp_size/length(unique(datCredit_AG[,get(clusVar)]))*100), "% of all performance spells.")
### RESULTS: 90k constitutes 14% of all loans
# Implied sampling fraction for the downsampling step
smp_perc <- smp_size/length(unique(datCredit_AG[,get(clusVar)]))
...
if (all(is.na(stratifiers))){ # No stratifiers
  ...
  datKeys <- unique(datCredit_AG[, mget(c(clusVar))])
  ...
  # Get unique subject IDs or keys from the full dataset
  datKeys <- unique(datCredit_AG[, mget(c(clusVar, stratifiers))])
...
datCredit_smp <- copy(datCredit_AG[get(clusVar) %in% datKeys_sampled[, get(clusVar),]])

# - Save intermediary snapshots to disk (zip) for quick disk-based retrieval later
pack.ffdf(paste0(genPath,"creditdata_final_AG_smp1a"), datCredit_smp)
pack.ffdf(paste0(genPath,"creditdata_final_AG_keys"), datKeys_sampled)
...
if (!exists('datCredit_smp')) unpack.ffdf(paste0(genPath,"creditdata_final_AG_smp1a"), tempPath)
...
pack.ffdf(paste0(genPath,"creditdata_final_AG_smp1b"), datCredit_smp)
# - Clean-up
...
pack.ffdf(paste0(genPath, "creditdata_final_AG_smp1b"), datCredit_smp); gc()
...
# - Confirm that required data objects are loaded into memory
if (!exists('datCredit_smp')) unpack.ffdf(paste0(genPath,"creditdata_final_AG_smp1b"), tempPath)
...
# - Save to disk (zip) for quick disk-based retrieval later
pack.ffdf(paste0(genPath, "creditdata_final_AG_smp1c"), datCredit_smp); gc()
...
# - Confirm that required data objects are loaded into memory
if (!exists('datCredit_smp')) unpack.ffdf(paste0(genPath,"creditdata_final_AG_smp1c"), tempPath)
...
# - Save to disk (zip) for quick disk-based retrieval later
pack.ffdf(paste0(genPath, "creditdata_final_AG_smp1d"), datCredit_smp); gc()
...
# - Confirm that required data objects are loaded into memory
if (!exists('datCredit_smp')) unpack.ffdf(paste0(genPath,"creditdata_final_AG_smp1d"), tempPath)
...
pack.ffdf(paste0(genPath,"creditdata_final_AG_smp1e"), datCredit_smp)
...
# - Confirm that required data objects are loaded into memory
if (!exists('datCredit_smp')) unpack.ffdf(paste0(genPath,"creditdata_final_AG_smp1e"), tempPath)
if (!exists('datMV')) unpack.ffdf(paste0(genPath,"datMV"), tempPath)
...
pack.ffdf(paste0(genPath,"creditdata_final_AG_smp2"), datCredit_smp)
...
# - Confirm that required data objects are loaded into memory
if (!exists('datCredit_smp')) unpack.ffdf(paste0(genPath,"creditdata_final_AG_smp2"), tempPath)
if (!exists('datKeys_sampled')) unpack.ffdf(paste0(genPath,"creditdata_final_AG_keys"), tempPath)
...
if (timeDef_TFD) {
  ...
  vLoanKeys_MultiSpell <- datCredit_train_AG %>% subset(PerfSpell_Num != 1,
                                                         c(get(clusVar), get(timeVar)))
  datCredit_train_AG <- datCredit_train_AG %>% subset(PerfSpell_Num == 1)
} else {
  datCredit_train_AG <- copy(datCredit_smp[get(clusVar) %in% dat_train_keys[, get(clusVar)],])
}

# - Extract the entire loan histories into the validation set for those remaining subjects
datCredit_valid_AG <- copy(datCredit_smp[!(get(clusVar) %in% dat_train_keys[, get(clusVar)]),]);gc()

# - [SANITY CHECKS]
if (timeDef_TFD) {
  # Can subsample be reconstituted?
  check.1 <- datCredit_smp[,.N] == datCredit_train_AG[,.N] + datCredit_valid_AG[,.N] + vLoanKeys_MultiSpell[,.N]# Should be TRUE
  # Does training set contain only first-time spells?
  check.2 <- datCredit_train_AG[PerfSpell_Num == 1,.N] == datCredit_train_AG[,.N] # Should be TRUE
  # Does validation spell contain spell numbers other than 1?
  (check.3 <- datCredit_valid_AG[PerfSpell_Num != 1,.N] > 0) # Should be TRUE
} else {
  # Can subsample be reconstituted?
...
# - Training dataset
pack.ffdf(paste0(genPath,"creditdata_train_AG"), datCredit_train_AG)

# - Validation dataset
pack.ffdf(paste0(genPath,"creditdata_valid_AG"), datCredit_valid_AG)
...
suppressWarnings(rm(dat_keys_smp_perf, dat_keys_smp_perf,  dat_train_keys_perf, dat_train_keys_def, datCredit_train_perf, datCredit_train_def,  datCredit_valid_perf, datCredit_valid_def,
                    check.4_a, check.4_b, check.4_c, check.5_a, check.5_b, datCredit_smp, datStrata_smp_min, datCredit_train_AG, datCredit_valid_AG));gc()
