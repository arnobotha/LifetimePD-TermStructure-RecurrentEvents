...
if (all(is.na(stratifiers))){ # No stratifiers
  ...
  datKeys <- unique(datCredit_PWPST[, mget(c(clusVar))])
  ...
  # Get unique subject IDs or keys from the full dataset
  datKeys <- unique(datCredit_PWPST[, mget(c(clusVar, stratifiers))])
...
datCredit_smp <- copy(datCredit_PWPST[get(clusVar) %in% datKeys_sampled[, get(clusVar),]])
