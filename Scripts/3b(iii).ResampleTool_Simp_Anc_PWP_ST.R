...
if (all(!is.na(stratifiers))){
  ...
  datStrata <- copy(datCredit_train_PWPST[Counter==1, ..selectionVar_train][, list(Freq = .N), by=stratifiers])
  ...
  (g1 <- ggplot(datStrata[get(stratifiers[1])>=minDate & get(stratifiers[1])<=maxDate,], aes(x=get(stratifiers[1]), y=Freq)) + theme_minimal() + 
      labs(x=bquote("Date "*italic(t)), y=bquote("Proporionate volume (%) of stratifiers within "*italic(D[T])~"("*.(round(datCredit_train_PWPST[,.N]/1000))*"k)")) + 
...
datCredit_train_PWPST[timeVar==timeVar_SpellMin, get(resolType)] %>% table() %>% prop.table()
datCredit_valid_PWPST[timeVar==timeVar_SpellMin, get(resolType)] %>% table() %>% prop.table()
### RESULTS: Proportions are fairly equal.
...
datCredit_train_PWPST[timeVar==timeVar_SpellMax, get(resolType)] %>% table() %>% prop.table()
datCredit_valid_PWPST[timeVar==timeVar_SpellMax, get(resolType)] %>% table() %>% prop.table()
### RESULTS: Proportions are fairly equal.
...
datAnnotate <- data.table(MAE = rep(0,anno_n*4), Mean_EventRate = rep(0, anno_n*4),
                          ...
                          vjust = c(5,6,7,9,19,20,21,21), hjust=rep(0.5, anno_n*4))
# Get the column names to help compute the MAEs
vCols <- colnames(datAggr_cohorts2)
...
###           indicating that the training and validation sets are well-representative of the full dataset. 
###           Although there seems to be a massive deviation between the training set and validation set from May 2022 until
###           the end of the period. The low MAE values are sufficient to conclude representativeness.
...
# --- Cleanup
suppressWarnings(rm(g1, g2, g3, g4, datCredit, datCredit_smp, stratifiers, clusVar, Counter, timeVar, End_Dte,
                    Start_Dte, datGraph, datCredit_train_PWPST, datCredit_valid_PWPST, 
